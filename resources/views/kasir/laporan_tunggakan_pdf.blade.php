...
<head>
    ...
    <title>Laporan Tunggakan - Periode {{ $periodeFormatted }}</title>
    <style>
...
<body>
    ...
    @php
        ...
        $groupedData = $data->sortBy(function($tagihan) {
            return $tagihan->status === 'Belum Lunas' ? 1 : 0;
        })->groupBy('status');
    ...
    <table>
        ...
        <tbody>
            @if(isset($groupedData['Belum Dibayarkan']) && $groupedData['Belum Dibayarkan']->isNotEmpty())
                <tr class="group-header">
                    <td colspan="7"><strong>Belum Dibayarkan</strong></td>
                </tr>
                @foreach ($groupedData['Belum Dibayarkan'] as $tagihan)
                    @php
                        $sisaPokok = $tagihan->sisa_pokok ?? $tagihan->jumlah_tagihan;
                        $totalBelumDibayar += $sisaPokok; // Total yang belum dibayar
                    @endphp
                    <tr>
                        <td>{{ $rowNumber++ }}</td>
            ...
            @if(isset($groupedData['Belum Lunas']) && $groupedData['Belum Lunas']->isNotEmpty())
                ...
                @foreach ($groupedData['Belum Lunas'] as $tagihan)
                    ...
                    <tr>
                        ...
                        <td>{{ $tagihan->tarif->nama_pembayaran ?? 'N/A' }}</td>
                        <td class="text-right">
                            <div>Total: Rp {{ number_format($tagihan->jumlah_tagihan, 0, ',', '.') }}</div>
                            <small style="color:#666;">Sudah dibayar: Rp {{ number_format($totalAngsuran, 0, ',', '.') }}</small><br>
                            <strong style="color:#dc2626;">Sisa: Rp {{ number_format($sisaPokok, 0, ',', '.') }}</strong>
                        </td>
                        <td class="text-center">
            ...
            @if(!isset($groupedData['Belum Dibayarkan']) && !isset($groupedData['Belum Lunas']))
                <tr>
                    <td colspan="7" class="text-center">Tidak ada data tunggakan untuk periode ini.</td>
                </tr>
            @endif

            <tr class="total-row">
                <th colspan="4" class="text-right">TOTAL BELUM DIBAYARKAN:</th>
                <td class="text-right">Rp {{ number_format($totalBelumDibayar, 0, ',', '.') }}</td>
                <td colspan="2"></td>
            </tr>
            <tr class="total-row">
                <th colspan="4" class="text-right">TOTAL SISA BELUM LUNAS:</th>
                <td class="text-right">Rp {{ number_format($totalBelumLunas, 0, ',', '.') }}</td>
                <td colspan="2"></td>
            </tr>
            <tr class="total-row">
                <th colspan="4" class="text-right">TOTAL KESELURUHAN TUNGGAKAN:</th>
                <td class="text-right">Rp {{ number_format($totalBelumDibayar + $totalBelumLunas, 0, ',', '.') }}</td>
                <td colspan="2"></td>
            </tr>
        </tbody>
    </table>
