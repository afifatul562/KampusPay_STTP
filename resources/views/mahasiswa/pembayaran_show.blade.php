...
@section('content')
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white p-6 rounded-xl shadow-md border border-gray-200">
            ...
            <form action="{{ route('mahasiswa.pembayaran.konfirmasi.store', $tagihan->tagihan_id) }}" method="POST" enctype="multipart/form-data" aria-label="Form upload bukti pembayaran">
                @csrf
                @if ($errors->any())
                ...
                <div class="mt-6">
                    <button type="submit" class="w-full inline-flex justify-center items-center px-4 py-3 border border-transparent text-sm font-medium rounded-lg text-white bg-gradient-to-r from-primary-600 to-primary-700 hover:from-primary-700 hover:to-primary-800 shadow-md hover:shadow-lg transition-all duration-200">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        Kirim Konfirmasi
                    </button>
                    <a href="{{ url()->previous() }}" class="block text-center mt-3 text-sm text-gray-600 hover:text-primary-600 transition-colors duration-200">
                        Batal
                    </a>
                </div>
            </form>
